  ...
  return (
    <div className="flex flex-col lg:flex-row p-4 sm:p-10 text-white min-h-full w-full gap-10">
      <div className="form h-full lg:w-[60%] rounded-2xl gap-6 flex flex-col">
        ...
        <div className="row2 ">
          <div className="field"><label className="form-label">Decimals <span className="hint">0–9</span></label><input className="input mono hint-label" name="decimals" placeholder="9" onChange={handleInputChange} /></div>
      ...
      <div className="p-4 h-full lg:w-[40%] bg-[#09090a] rounded-2xl">
        <aside className="aside flex flex-col text-[#9a9aa8] gap-3" >
          ...
          >
            ...
            <div className="tc-logo bg-gradient-to-b from-[#FF8A3D] to-[#E5500A] rounded-full w-10 h-10 flex justify-center items-center text-black font-bold mr-2">
              {!imageError && tokenData.image ? (
                <img
                  src={tokenData.image}
                  alt={tokenData.name}
                ...
                <span className="text-lg font-bold text-white">
                  {tokenData.symbol?.[0] ?? tokenData.name?.[0] ?? "?"}
                </span>
              )}
            </div>            <div className="tc-rows bg-black rounded-2xl">
              <div className="tc-row"><span className="block">Decimals</span><b className="text-white block">{tokenData.decimal}</b></div>
              <div className="tc-row"><span>Supply</span><b className="text-white block">{tokenData.supply}</b></div>
              <div className="tc-row"><span>Mint authority</span><b className="text-white block">{trimPublicKey(new PublicKey(tokenData.authority))}</b></div>
